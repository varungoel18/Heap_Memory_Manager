...
static vm_page_for_families_t *first_vm_page_for_families = NULL; /* Most recent vm page that has been requested from kernel to host page families */
...
/* Function/API to request a VM Page from kernel */
static void *mm_get_new_vm_page_from_kernel(int units)
{
     char *vm_page = mmap(0, units * SYSTEM_PAGE_SIZE, PROT_READ| PROT_WRITE | PROT_EXEC, MAP_ANON | MAP_PRIVATE, 0 , 0); /* NULL, Amount of memory for allocation , Read write and execute permission , map is Anonymous or private , NULL , NULL */
...
/* Function/API to return a page to kernel */
...
static inline uint32_t mm_max_page_allocatable_memory()
{
    ...
    return (uint32_t)(SYSTEM_PAGE_SIZE - offset_of(vm_page_t, page_memory));
}
...
vm_page_t *allocate_vm_page(vm_page_family_t *vm_page_family)
{
     ...
     /* Initialize lower most meta block in vm page */
     ...
     /* Set th back pointer to page family */
...
static vm_page_t *mm_family_new_page_add(vm_page_family_t *vm_page_family)
{
    ...
    /* The new page is like one free block, add it to the free block list */
...
static block_meta_data_t *mm_free_blocks(block_meta_data_t *to_be_free_block)
{
    ...
    if(next_block)
    {
        /* Scenario 1 : When data block to be freed is not the last upper most meta block in a VM data page*/
        to_be_free_block->block_size += mm_get_hard_internal_memory_frag_size (to_be_free_block, next_block);
    }
    else
    {
        /* Scenario 2: Page Boundry condition Block being freed is the upper most free data block in a VM data page, check of hard internal fragmented memory and merge*/
...
void mm_print_vm_page_details(vm_page_t *vm_page)
{
    ...
    {
        printf("\t\t\t%-14p Block %-3u %s  block_size = %-6u  offset = %-6u  prev = %-14p  next = %p\n", curr, j++, curr->is_free ? "F R E E D" : "ALLOCATED",
...
void mm_print_memory_usage(char *struct_name)
{
    ...
    printf("# Of VM Pages in Use : %u (%lu Bytes)\n", cumulative_vm_pages_claimed_from_kernel, SYSTEM_PAGE_SIZE * cumulative_vm_pages_claimed_from_kernel);
